...
public class PostService {
    ...
    public Long create(User user, PostCreateRequest postCreateRequest) {
        //TODO : 리팩토링 필요
        Groups group = getGroupById(postCreateRequest.getGroupId());
        validateGroupParticipant(user, group);
        Post post = postRepository.save(Post.create(user, group, postCreateRequest.toEntity()));
        if (!Objects.isNull(postCreateRequest.getImageUrls())) {
            List<PostImage> postImages = postCreateRequest.getImageUrls().stream()
    ...
    public void validateGroupParticipant(User user, Groups group) {
        if (!participantRepository.existsByUserAndGroup(user, group)) {
            throw new CustomException(ErrorCode.GROUP_PARTICIPANT_NOT_FOUND);
        }
    }
}
